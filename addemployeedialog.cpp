...
void AddEmployeeDialog::insertEmployee(const QString& first, const QString& last, const QString& middle,
    ...
    QSqlQuery deptQuery;
    QString deptSql = loadSqlQuery(":/sql/getDepartmentIdByUserId.sql");
    ...
    QString insertSql = loadSqlQuery(":/sql/insertUser.sql");
    ...
    if (!insertQuery.exec()) {
        if (insertQuery.lastError().text().contains("Duplicate entry") &&
            ...
            qDebug() << "Ошибка SQL:" << insertQuery.lastError().text();
